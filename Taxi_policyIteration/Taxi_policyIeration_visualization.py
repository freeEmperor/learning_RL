...
if __name__ == '__main__':
    ...
    for i in range(0, round//2):
        ...
        ax.set_title( f'State value with: passenger in {loc[pas]}, destination is {loc[des]}, round {i+1}')
        plt.savefig(f'./figure/figure{i+1}.jpg',dpi = 100)
        plt.close()

    with imageio.get_writer(uri=f'state_value_iteration({pas},{des}).gif', mode='I', fps=3) as writer:
        for i in range(0, round//2):
